## v0.03.40 - Correct passphrase strength badge, add regression tests, speed up word picking
- The strength badge estimated passphrase entropy the same way as random-character passwords (character-class alphabet size x length), which drastically overstated it -- a 3-word passphrase with a trailing number was being called "very strong" from character variety alone. It now uses log2(dictionary size) x word count (+ log2(10) if a number is included) for passphrase mode, matching how word-list/Diceware-style entropy is normally reported.
- Added `test/regression.js`, a dependency-free headless-Chrome harness covering the URL/API parameters and passphrase generation (duplicate words, exact length, tight-budget fallback, the prefix/XSS/hex-clobber bugs fixed earlier this series). See the Testing section in README. All 9 checks pass.
- Passphrase word selection no longer filters an entire length-bucket (up to ~5,300 words) on every candidate check just to exclude the handful of already-used words in the current passphrase; it now tries a few random draws first and only falls back to filtering for small or mostly-exhausted buckets.
